...
| # | 画面名 | パス | 概要 |
|---|---|---|---|
| S1 | ホーム(レシピ検索) | `/` | 食材とジャンルを指定してAIにレシピを考案してもらい、結果を表示・お気に入り登録・再提案する |
| S2 | お気に入り一覧 | `/favorites` | 保存済みのお気に入りレシピを折りたたみ形式で一覧表示し、ジャンルで絞り込み・削除できる |
...
2. ジャンル選択(選択式。和食・洋食・中華・イタリアン・韓国料理・お弁当・こだわりなし)
3. 「レシピを考えてもらう」ボタン
4. 生成結果表示エリア(料理名・ジャンル・調理時間・人数・材料・作り方)。未生成時は非表示
   - 材料はチェックボックス形式で表示し、調理中に準備できたものをチェックできる(保存はしない)
5. 生成結果が表示されている間、以下の2つのボタンを表示
   - 「お気に入り登録」ボタン
   - 「別のレシピを提案してもらう」ボタン(同じ食材・ジャンルのまま、直前とは別のレシピを再生成する)

### 2.4 S2(お気に入り一覧)の画面構成

- ジャンルで絞り込むためのフィルター(「すべて」+ジャンルごとのボタン)を上部に表示
- 保存済みのレシピをカード形式で一覧表示する。**通常はタイトル・ジャンルのみを表示し、
  タップすると調理時間・人数・使った食材・材料・作り方が展開表示される**(折りたたみ式)
- 各カードに「削除」ボタンを表示(折りたたみ状態に関わらず操作可能)
- 保存件数が0件の場合は「まだお気に入りがありません」、絞り込みで0件の場合は
  「このジャンルのお気に入りはありません」と表示
...
- `genre`はお気に入り一覧のジャンル絞り込み(F11)に使用する。生成時に選択したジャンルをそのまま保存する
  (AIが実際にどのジャンルの料理を作ったかではなく、利用者が指定した意図を優先する)
...
- 「別のレシピを提案してもらう」(F12)は、直前に表示していたレシピ名をプロンプトに含め、
  「その料理とは別のものにしてください」と明示的に指示することで、同じ提案が繰り返される可能性を下げる
  (完全に重複しないことを保証するものではない)
- 材料のチェックボックス(F9)の状態は、ブラウザの画面上のみで保持し、データベースには保存しない
  (ページを再読み込みすると状態は消える。買い物・調理中の一時的な用途のため、永続化は不要と判断)
